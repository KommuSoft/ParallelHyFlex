package parallelhyflex.logging;

/**
 *
 * @author kommusoft
 */
public final class LoggingParameters {

    public static final boolean LOG_START = true;
    public static final boolean LOG_MEMORY_SET = true;
    public static final boolean LOG_MEMORY_BETTER = true;
    public static final boolean LOG_NEGOTI_START = true;
    public static final boolean LOG_NEGOTI_STOP = true;
    public static final boolean LOG_ADAPHH_BLST = true;
    public static final boolean LOG_ADAPHH_EXRC = true;
    public static final boolean LOG_ADAPHH_PARC = true;
    public static final boolean LOG_SEASPA_SETV = true;
    public static final boolean LOG_STTEXC = true;
    public static final String LOG_START_TEXT = "ST";
    public static final String LOG_MEMORY_SET_TEXT = "MS %s %s";
    public static final String LOG_MEMORY_BETTER_TEXT = "MB %s %s";
    public static final String LOG_NEGOTI_START_TEXT = "NS";
    public static final String LOG_NEGOTI_STOP_TEXT = "NP";
    public static final String LOG_STTEXC_TEXT = "XC";
    public static final String LOG_SEASPA_SETV_TEXT = "SS %s";
    public static final String LOG_ADAPHH_BLST_TEXT = "ADAPHH-BLST %s";
    public static final String LOG_ADAPHH_EXRC_TEXT = "ADAPHH-EXRC %s %s %s";
    public static final String LOG_ADAPHH_PARC_TEXT = "ADAPHH-PARC %s %s";

    private LoggingParameters() {
    }
}
