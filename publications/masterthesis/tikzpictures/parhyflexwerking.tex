...
\draw[thick,dotted,->] (SGI) -- (SI0);
\draw[dashed,->] (MEM1) .. controls ($(MEM1)+(0,-1)$) and ($(SGI)+(1,0)$) .. (SGO);
...
\draw[dashed,->] (NGPIN) .. controls ($(NGPIN.south)+(0,-1)$) and ($(NGO)+(1,0)$) .. (NGO);
\draw[dotted,thick,<-] (NGNIN) .. controls ($(NGNIN.north)+(0,1)$) and ($(NGIVNI)+(0.5,0.5)$) .. (NGIVNI) .. controls ($(NGIVNI)+(-0.5,-0.5)$) and ($(NGI)+(1,0)$) .. (NGI);
