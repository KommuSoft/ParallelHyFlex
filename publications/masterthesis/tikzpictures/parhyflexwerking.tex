...
\node [minimum width=\sc*2.5 cm,minimum height=\sc*2.35 cm,draw=black] (NG) at (2,0.25) {};
...
\coordinate (NGPI) at ($(NG.north west)+(0.5,0)$);
\coordinate (NGNI) at ($(NG.north east)+(-0.5,0)$);
\coordinate (NGIVNI) at ($(NG.north west)+(-0.5,0.5)$);
\coordinate (NGO) at ($(NG.west)+(-2,0)$);
\coordinate (NGI) at ($(NGO)+(0,0.5)$);
\draw[->] (ESHO) .. controls ($(ESHO)+(0,1)$) and ($(NGPI)+(0,1)$) .. (NGPI);
\draw (NG.north) -- (NG.south);
\draw[dashed,->] (NGPI) .. controls ($(NGPI)+(0,-1)$) and ($(NGO)+(1,0)$) .. (NGO);
\draw[dotted,thick,<-] (NGNI) .. controls ($(NGNI)+(0,1)$) and ($(NGIVNI)+(0.5,0.5)$) .. (NGIVNI) .. controls ($(NGIVNI)+(-0.5,-0.5)$) and ($(NGI)+(1,0)$) .. (NGI);
\draw[dotted,thick,<-] (ESHI) .. controls ($(ESHI)+(0,1.25)$) and ($(NGIVNI)+(0.5,0.5)$) .. (NGIVNI);
\draw[->] (NGPI) .. controls ($(NGPI)+(0,-1)$) and ($(PSS.north)+(0,1)$) .. (PSS.north);
\end{scope}
\end{tikzpicture}
\end{center}