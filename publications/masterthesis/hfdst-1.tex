...
\begin{equation}
\mean{\theta}\approx 1+\vec{\hat{\alpha}}_0\cdot\onematrix+s\cdot\brak{\displaystyle\sum_{i=0}^{\infty}\lambda_{1,\hat{P}}^i}=1+\vec{\hat{\alpha}}_0\cdot\onematrix+\displaystyle\frac{s}{1-\lambda_{1,\hat{P}}}
\end{equation}
Net zoals we de termen van dit gemiddelde benadert hebben aan de hand van \eqnref{longTermAlpha}, kunnen we de eerste termen benaderen door deze vergelijking omgekeerd toe te passen. We stellen dus dat $1+\vec{\hat{\alpha}}_0\cdot\onematrix=s\cdot\brak{\lambda^{-1}+\lambda^{-2}}$. Dit levert ons een ruwe schatting voor de raaktijd op:
\begin{equation}
\mean{\theta}=\displaystyle\frac{s}{\brak{1-\lambda_{1,\hat{P}}}\cdot\lambda_{1,\hat{P}}^2}
\end{equation}
...
We defini\"eren de parallelle raaktijd $\phittime$ van een Markov-keten $\PopChain$ die bestaat uit $p$-tuples van populaties populaties $\tupl{\PopSet_{t,1},\PopSet_{t,2},\ldots,\PopSet_{t,p}}$ als:
\begin{equation}
...
&\approx 1+\brak{\vec{\hat{\alpha}}_0\cdot\onematrix}^p+s^p\cdot\brak{\displaystyle\sum_{i=0}^{\infty}\lambda_{1,\hat{P}}^{p\cdot i}}=1+\brak{\vec{\hat{\alpha}}_0\cdot\onematrix}^p+\displaystyle\frac{s^p}{1-\lambda_{1,\hat{P}}^p}\\
&\approx\displaystyle\frac{s^p}{\brak{1-\lambda_{1,\hat{P}}^p}\cdot\lambda_{1,\hat{P}}^{2p}}
%\Prob{\phittime\geq t}&=\Prob{\hittime_1\geq t\wedge\hittime_2\geq t\wedge\ldots\wedge\hittime_p\geq t}\\
%&=\Prob{\hittime_1\geq t}\cdot\Prob{\hittime_2\geq t}\cdot\ldots\cdot\Prob{\hittime_p\geq t}\\
%&=\Prob{\hittime\geq t}\cdot\Prob{\hittime\geq t}\cdot\ldots\cdot\Prob{\hittime\geq t}=\Prob{\hittime\geq t}^p
\end{aligned}
...
\begin{equation}
\mbox{speed-up}=\displaystyle\frac{\mean{\phittime}}{\mean{\hittime}}=\displaystyle\frac{s^{p-1}}{\lambda_{1,\hat{P}}^{2p-2}}\cdot\displaystyle\frac{1-\lambda_{1,\hat{P}}}{1-\lambda_{1,\hat{P}}^{p}}
\eqnlab{speedupMetaheuristic}
\end{equation}
\subsubsection{Semantiek van parameters $\lambda$ en $s$}
