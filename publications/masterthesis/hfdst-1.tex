...
\begin{equation}
\begin{array}{cc}
\vec{x}\cdot\hat{P}^t\cdot\onematrix[1\times N-g]\approx\vec{x}\cdot\lambda_{1,\hat{P}}^t\cdot\vec{v}_{\hat{P}}=\lambda_{1,\hat{P}}^t\cdot\sigma&\mbox{($t\gg 1$)}
\end{array}
\eqnlab{longTermAlpha}
\end{equation}
Wanneer we als $\vec{x}$ de begindistributie $\vec{\hat{\alpha}}_0$ nemen, kunnen we het dot-product tussen $\vec{\hat{\alpha}}_0$ en $\vec{v}_{\hat{P}}$ defini\"eren als $\sigma$. We kunnen vervolgens \eqnref{meanHit} en \eqnref{longTermAlpha} samennemen in een nieuwe vergelijking die ons met behulp van \theref{eigenValuesMatrix} een benaderende waarde voor de gemiddelde raaktijd oplevert:
\begin{equation}
...
\begin{equation}
\mean{\hittime}=\displaystyle\frac{\sigma}{\brak{1-\lambda_{1,\hat{P}}}\cdot\lambda_{1,\hat{P}}}
...
\begin{equation}
\mbox{speed-up}=\displaystyle\frac{\mean{\hittime}}{\mean{\phittime}}=\displaystyle\frac{\lambda_{1,\hat{P}}^{p-1}}{\sigma^{p-1}}\cdot\displaystyle\frac{1-\lambda_{1,\hat{P}}^{p}}{1-\lambda_{1,\hat{P}}}
\eqnlab{speedupMetaheuristic}
\end{equation}
\subsubsection{Semantiek van parameters $\sigma$ en $\lambda$}
\paragraph{De factor $\sigma$}
\eqnref{speedupMetaheuristic} formaliseert de verwachte \emph{speed-up} bij een probleem. Deze formule bevat 3 parameters en kan daarom niet zomaar ge\"interpreteerd worden. Behalve het aantal parameters $p$ is het niet triviaal om de overige parameters te interpreteren. $\sigma$ hebben we gedefinieerd als het dot-product tussen $\vec{\hat{a}}_0$ en $\vec{v}_{\hat{P}}$. Dit kunnen we dus interpreteren als een getal proportioneel met de cosinus tussen de hoek tussen beide vectoren ofwel in welke mate de \emph{initi\"ele} distributie gelijkloopt met de distributie van populaties na een lange tijd. Vermits we over (gedeeltelijke) probabiliteitsvectoren spreken, kunnen we bovendien grenzen zetten op $\sigma$:
\begin{equation}
0\leq \sigma<1
\eqnlab{boundsS}
\end{equation}
$\sigma$ kan echter nooit gelijk zijn aan $1$ omdat voor elke redelijke kansverdeling er ook een (kleine) kans bestaat dat we meteen in een doel-populatie terechtkomen. Verder spreekt \eqnref{boundsS} over de theoretische grenzen van $\sigma$. In de praktijk verwachten we andere grenzen. Een redelijker scenario is dat alle populaties (zowel doel-populaties als ordinaire populaties) met eenzelfde kans voorkomen. De argumentatie is dat een initi\"ele oplossingsgenerator meestal volgens een uniforme kansverdeling een oplossing $\sol$ uit de oplossingsruimte $\SolSet$ genereert. Vermits populaties een vaste grote hebben is elke populatie ook even waarschijnlijk. In dat geval geldt dus:
\begin{equation}
\begin{array}{cc}
0\leq \sigma\leq\displaystyle\frac{\brak{N-g}}{N\cdot\brak{N-g}}=\displaystyle\frac{1}{N}&\mbox{(uniforme initi\"ele verdeling)}
\end{array}
\end{equation}
