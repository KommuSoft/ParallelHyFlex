...
\begin{algorithm}[H]
 ...
 \Repeat{het stopcriterium is bereikt}{
  ...
  $b_{t+1}\leftarrow\displaystyle\argmin_{\sol\in\PopSet_t\cup\accl{b_t}}{\fun{\evalfun}{\sol}}$\;
  $t\leftarrow t+1$\;
 }
 \KwRet{$b_t$}
...
\eqnlab{exhaust}
\end{equation}
Een ander logische gevolg uit \eqnref{oneStepTransition} zijn de transities van ordinaire populaties naar andere ordinaire populaties. De som van de kansen die in deze deelvector aanwezig zijn, is de kans dat we na deze $t$ stappen nog steeds geen acceptabele oplossing gevonden hebben:
\begin{eqnarray}
...
\begin{equation}
\begin{array}{cc}
\vec{x}\cdot\hat{P}^t\cdot\onematrix[1\times N-g]\approx\vec{x}\cdot\lambda_{1,\hat{P}}^t\cdot\vec{v}_{\hat{P}}=\lambda_{1,\hat{P}}^t\cdot s&\mbox{($t\gg 1$)}
\end{array}
\eqnlab{longTermAlpha}
\end{equation}
Wanneer we als $\vec{x}$ de begindistributie $\vec{\hat{\alpha}}_0$ nemen, kunnen we het dot-product tussen $\vec{\hat{\alpha}}_0$ en $\vec{v}_{\hat{P}}$ defini\"eren als $s$. We kunnen vervolgens \eqnref{meanHit} en \eqnref{longTermAlpha} samennemen in een nieuwe vergelijking die ons met behulp van \theref{eigenValuesMatrix} een benaderende waarde voor de gemiddelde raaktijd oplevert:
\begin{equation}
\mean{\theta}\approx 1+\vec{\hat{\alpha}}_0\cdot\onematrix+s\cdot\brak{\displaystyle\sum_{i=1}^{\infty}\lambda_{1,\hat{P}}^i}=1+\vec{\hat{\alpha}}_0\cdot\onematrix+\displaystyle\frac{s\cdot \lambda_{1,\hat{P}}}{1-\lambda_{1,\hat{P}}}
\end{equation}
Net zoals we de termen van dit gemiddelde benadert hebben aan de hand van \eqnref{longTermAlpha}, kunnen we de eerste termen benaderen door deze vergelijking omgekeerd toe te passen. We stellen dus dat $1+\vec{\hat{\alpha}}_0\cdot\onematrix=s\cdot\brak{1+\lambda^{-1}}$. Dit levert ons een ruwe schatting voor de raaktijd op:
\begin{equation}
\mean{\hittime}=\displaystyle\frac{s}{\brak{1-\lambda_{1,\hat{P}}}\cdot\lambda_{1,\hat{P}}}
...
&\approx 1+\brak{\vec{\hat{\alpha}}_0\cdot\onematrix}^p+s^p\cdot\brak{\displaystyle\sum_{i=1}^{\infty}\lambda_{1,\hat{P}}^{p\cdot i}}=1+\brak{\vec{\hat{\alpha}}_0\cdot\onematrix}^p+\displaystyle\frac{s^p\cdot\lambda_{1,\hat{P}}^p}{1-\lambda_{1,\hat{P}}^p}\\
&\approx\displaystyle\frac{s^p}{\brak{1-\lambda_{1,\hat{P}}^p}\cdot\lambda_{1,\hat{P}}^{p}}
%\Prob{\phittime\geq t}&=\Prob{\hittime_1\geq t\wedge\hittime_2\geq t\wedge\ldots\wedge\hittime_p\geq t}\\
%&=\Prob{\hittime_1\geq t}\cdot\Prob{\hittime_2\geq t}\cdot\ldots\cdot\Prob{\hittime_p\geq t}\\
%&=\Prob{\hittime\geq t}\cdot\Prob{\hittime\geq t}\cdot\ldots\cdot\Prob{\hittime\geq t}=\Prob{\hittime\geq t}^p
\end{aligned}
...
\begin{equation}
\mbox{speed-up}=\displaystyle\frac{\mean{\hittime}}{\mean{\phittime}}=\displaystyle\frac{\lambda_{1,\hat{P}}^{p-1}}{s^{p-1}}\cdot\displaystyle\frac{1-\lambda_{1,\hat{P}}^{p}}{1-\lambda_{1,\hat{P}}}
\eqnlab{speedupMetaheuristic}
\end{equation}
\subsubsection{Semantiek van parameters $s$ en $\lambda$}
\paragraph{De factor $s$}
\eqnref{speedupMetaheuristic} formaliseert de verwachte \emph{speed-up} bij een probleem. Deze formule bevat 3 parameters en kan daarom niet zomaar ge\"interpreteerd worden. Behalve het aantal parameters $p$ is het niet triviaal om de overige parameters te interpreteren. $s$ hebben we gedefinieerd als het dot-product tussen $\vec{\hat{a}}_0$ en $\vec{v}_{\hat{P}}$. Dit kunnen we dus interpreteren als een getal proportioneel met de cosinus tussen de hoek tussen beide vectoren ofwel in welke mate de \emph{initi\"ele} distributie gelijkloopt met de distributie van populaties na een lange tijd. Vermits we over (gedeeltelijke) probabiliteitsvectoren spreken, kunnen we bovendien grenzen zetten op $s$:
\begin{equation}
0\leq s<1
\eqnlab{boundsS}
\end{equation}
$s$ kan echter nooit gelijk zijn aan $1$ omdat voor elke redelijke kansverdeling er ook een (kleine) kans bestaat dat we meteen in een doel-populatie terechtkomen. Verder spreekt \eqnref{boundsS} over de theoretische grenzen van $s$. In de praktijk verwachten we andere grenzen. Een redelijker scenario is dat alle populaties (zowel doel-populaties als ordinaire populaties) met eenzelfde kans voorkomen. De argumentatie is dat een initi\"ele oplossingsgenerator meestal volgens een uniforme kansverdeling een oplossing $\sol$ uit de oplossingsruimte $\SolSet$ genereert. Vermits populaties een vaste grote hebben is elke populatie ook even waarschijnlijk. In dat geval geldt dus:
\begin{equation}
\begin{array}{cc}
0\leq s\leq\displaystyle\frac{\brak{N-g}}{N\cdot\brak{N-g}}=\displaystyle\frac{1}{N}&\mbox{(uniforme initi\"ele verdeling)}
\end{array}
\end{equation}
\paragraph{De factor $\lambda$}
$\lambda$ is de grootste eigenwaarde van de matrix $\hat{P}$ en geeft een aanwijzing hoe snel een willekeurige vector naar zijn overeenkomstige eigenvector $\vec{v}$ zal convergeren: bij een $\lambda\rightarrow 1$ verwachten we dat we sneller zullen convergeren. Wat dus uit \eqnref{hitTimeSingle} naar voren komt is dat we op zoek gaan naar een optimale convergentiesnelheid om vanuit de initi\"ele probabiliteitsvector $\vec{\hat{\alpha}}_0$ naar de uiteindelijke waarde $\vec{v}$.
\begin{figure}
...
\foreach \s/\t in {0.1/,0.05/dashed,0.025/dotted} {
  \draw[\t] plot[id=x] function{\s/(x*(1-x))} node[right] {$s=\s$};
}
\end{tikzpicture}
