...
\begin{algorithm}[hbt]
 ...
 \Func($\funm{gereed}{}$){
   \Return{$z\geq d\wedge\impt\fbrk{d}$}\;
 }
 \Func($\funm{reset}{}$){
   $\impt\leftarrow\funm{array}{d+1,\fals}$\;
   $z\leftarrow 0$\;
 ...
 \Func($\funm{zendData}{}$){
   \While{\emph{$z<d\wedge\impt\fbrk{z}$}}{
	 $l\leftarrow 2^z$\;
	 $\rcvr\leftarrow \idet\mbox{ XOR }l$\;
	 \eIf{\emph{$\rcvr < p$}}{
	   $\bset\leftarrow\idet\mbox{ AND }\brak{\mbox{NEG } l-1}$\;
	   $m\gets\funm{min}{l,p-\bset}$\;
	   $\msgt\leftarrow\funm{array}{m}$\;
	   $\msgt\fbrk{0:m-1}\leftarrow\rslt\fbrk{\bset:\bset+m-1}$\;
	   $\funm{isend}{\rcvr,\msgt}$\;
	 }{
	   $\impt\fbrk{z+1}\leftarrow\true$
	 }
	 $z\leftarrow z+1$\;
   }
 }{}
